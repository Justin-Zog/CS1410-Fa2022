...
public class SpellChecker {
    public static void main(String[] args) {
        ...
        try (Scanner input = new Scanner(file)) {
            while (input.hasNextLine()) {
                ...
                // Take the punctuation off of the words then check if they are in the dictionary.
                String[] words = line.replaceAll("\\p{P}", "").toLowerCase().split(" ");
                for (String word : words) {
                    if (!dictionary.search(word) && word.length() > 0) {
    ...
    public static BinarySearchTree<String> readDictionary() {
        ...
        // Shuffle the words so they aren't in alphabetical order.
